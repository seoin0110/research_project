...
class TFTacotronEmbeddings(tf.keras.layers.Layer):
    ...
    def call(self, inputs, training=False):
        ...
        # 기존 코드
        input_embeddings = self._embedding(inputs, training=training)
...
class TFTacotron2(BaseModel):
    ...
    def call(
        ...
        **kwargs,
        ...
        tf.print(f"Encoder 처리 시간: {encoder_time}초")
        tf.print(f"Encoder Hidden States Shape: {tf.shape(encoder_hidden_states)}")
        ...
        tf.print(f"Decoder 처리 시간: {decoder_time}초")
        ...
        postnet_time = time.time() - start_time
        tf.print(f"Postnet 처리 시간: {postnet_time}초")
        ...
        tf.print(f"Mel Outputs Shape: {tf.shape(mel_outputs)}")
        tf.print(f"Stop Token Prediction Shape: {tf.shape(stop_token_prediction)}")
        tf.print(f"Alignment History Shape: {tf.shape(alignment_history)}")
    ...
    def inference(self, input_ids, input_lengths, speaker_ids, **kwargs):
        ...
        tf.print(f"Mel Outputs Shape: {tf.shape(mel_outputs)}")
        tf.print(f"Stop Token Prediction Shape: {tf.shape(stop_token_predictions)}")
        tf.print(f"Alignment History Shape: {tf.shape(alignment_historys)}")
