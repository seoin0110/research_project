...
class TFMelGANGenerator(BaseModel):
    ...
    def call(self, mels, **kwargs):
        ...
        result = self.inference(mels)
        end_time = time.time()  # 시간 측정 종료
        tf.print(f"MelGAN Generator 처리 시간: {end_time - start_time}초")
        tf.print(f"MelGAN Generator call 결과 shape: {tf.shape(result)}")
        return result
    @tf.function(
        input_signature=[
    ...
    def inference(self, mels):
        start_time = time.time()  # 시간 측정 시작
        result = self.melgan(mels)
        end_time = time.time()  # 시간 측정 종료
        tf.print(f"MelGAN Generator inference 처리 시간: {end_time - start_time}초")
        tf.print(f"MelGAN Generator inference 결과 shape: {tf.shape(result)}")
        return result
    @tf.function(
        input_signature=[
...
class TFMelGANMultiScaleDiscriminator(BaseModel):
    ...
    def call(self, x, **kwargs):
        ...
        for f in self.discriminator:
            ...
            x = self.pooling(x)
        end_time = time.time()  # 시간 측정 종료
        tf.print(f"MelGAN Multi-Scale Discriminator 처리 시간: {end_time - start_time}초")
        tf.print(f"MelGAN Multi-Scale Discriminator call 결과 shape: {[[tf.shape(out) for out in scale_out] for scale_out in outs]}")
        return outs